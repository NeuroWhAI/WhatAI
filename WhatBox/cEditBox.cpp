...
int cEditBox::OnRender()
{
	...
	SIZE Size = cCore::Resource.Font0.GetStringSize(&wStr);
	...
	while(wStr.size() > 0  &&  Size.cx > m_Size.x)
	{
		...
		Size = cCore::Resource.Font0.GetStringSize(&wStr);
	}
	...
	cCore::Resource.Font0.ShowText(wStr, CAST(m_Pos.x, int) + 4, CAST(m_Pos.y, int) + 2, Color);


	/* 커서 */
	if(m_bShowCursor)
	{
		cCore::Resource.Font0.ShowText("｜",
			CAST(m_Pos.x, int) + Size.cx - 13,
			CAST(m_Pos.y, int) + 2,
			Color);
	}
...
int cEditBox::SetStr(const std::wstring& text)
{
	m_pTextBox->SetStr(text);
